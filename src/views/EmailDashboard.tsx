...
export default function EmailDashboard() {
  ...
  useEffect(() => {
    ...
    fetchEmailLogs(true);
    fetchEmailStats();
  }, [filters.status, filters.template_id, filters.recipient, filters.from_date, filters.to_date, authLoading, isAdmin]);

  const fetchOnboardingConfig = async () => {
    try {
      const response = await apiClient.get('/admin/onboarding/config');
  ...
  const updateOnboardingConfig = async (payload: Partial<OnboardingConfig>) => {
    try {
      setOnboardingSaving(true);
      const response = await apiClient.put('/admin/onboarding/config', payload);
      if (response.success && response.data) {
        setOnboardingConfig(response.data);
        toast({
          title: 'Updated',
      ...
      const msg = error instanceof Error ? error.message : 'Failed to update onboarding config';
      toast({
        title: 'Error',
        description: msg,
        variant: 'destructive',
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <Label className="text-sm font-medium">Enable onboarding flow</Label>
                    <p className="text-xs text-muted-foreground mt-1">
                      When enabled, only users created after enabled time will see onboarding.
                    </p>
                  </div>
                  <Switch
                    checked={Boolean(onboardingConfig?.enabled)}
                    disabled={onboardingSaving || !onboardingConfig}
                    onCheckedChange={(checked) =>
                      updateOnboardingConfig({
                        enabled: checked,
                        enabledAt: checked ? new Date().toISOString() : onboardingConfig?.enabledAt || null,
                      })
                    }
                  />
                </div>
                ...
                <div>
                  <Label className="text-sm font-medium">Current enabledAt</Label>
                  <p className="text-sm text-muted-foreground mt-1">
                    {onboardingConfig?.enabledAt
                      ? new Date(onboardingConfig.enabledAt).toLocaleString()
                      : 'Not set'}
                  </p>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  >
                    Re-enable for new users from now
                  </Button>
                  <Button
                    variant="ghost"
                    disabled={onboardingSaving}
                    onClick={fetchOnboardingConfig}
                  >
                    Refresh
                  </Button>
                </div>
                ...
                <div className="pt-2 border-t">
                  <Label className="text-sm font-medium">Tour steps</Label>
                  <p className="text-xs text-muted-foreground mt-1">
                    Uncheck a step to exclude it from the in-dashboard guide.
                  </p>

                  <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {[
                      { key: 'notificationsBell', label: 'Notifications bell' },
                      { key: 'createPost', label: 'Create (Agent)' },
                      { key: 'generationDemo', label: 'Generation + Scheduling demo' },
                      { key: 'scheduledPosts', label: 'Scheduled Posts' },
                      { key: 'media', label: 'Media Library' },
                      { key: 'settings', label: 'Settings' },
                    ].map((s) => {
                      const steps = onboardingConfig?.tourSteps || {};
                      const checked = steps[s.key] !== false;
                      return (
                        <div key={s.key} className="flex items-center justify-between rounded-lg border p-3">
                          ...
                          <Switch
                            ...
                            onCheckedChange={(next) => {
                              const current = onboardingConfig?.tourSteps || {};
                              updateOnboardingConfig({
                                tourSteps: { ...current, [s.key]: next },
                              });
                            }}
                          />
                        </div>
