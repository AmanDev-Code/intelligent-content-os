...
export default function Auth() {
  ...
  // Show toast when redirected back from a failed Google OAuth attempt.
  // Guard: if the user already has a session (e.g. supabase-js processed the
  // hash before the callback page could redirect cleanly), skip the error — the
  // session effect above will redirect them to the dashboard momentarily.
  useEffect(() => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (!url) {
        throw new Error(response?.message || "No redirect URL returned from server.");
      }
