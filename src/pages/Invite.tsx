...
export default function Invite() {
  ...
  useEffect(() => {
    if (token) {
      document.title = "You're Invited - Postra";
    }
  }, [token]);
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background p-6">
      <div className="w-full max-w-sm space-y-6 text-center">
        ...
        <h2 className="text-xl font-semibold">You're invited to Postra</h2>
        <p className="text-muted-foreground text-sm">
          Create your account to get started with AI-powered content creation.
        </p>
        <Button asChild className="w-full">
          <Link to={`/auth?invite=${encodeURIComponent(token)}`}>Create account</Link>
        </Button>
      </div>
    </div>
  );
